...
- **本記事の設定例のキー名（`regex` / `url`）やキャプチャ参照（`$0` / `$1`）は、一般的な慣習に基づく記述です。** changelogには「正規表現でマッチしたリンクバッジ」とあるのみで、正確なスキーマは未公開です。実際のキー名・記法は公式の[Settingsドキュメント](https://code.claude.com/docs/en/settings)で確認してください。
